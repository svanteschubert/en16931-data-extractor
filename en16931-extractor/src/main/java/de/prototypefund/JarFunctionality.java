...
import de.prototypefund.en16931.OdtTableDataExtraction;
...
public class JarFunctionality {
    ...
    public static void main(String[] args) {
        if(args == null || args.length == 0){
            ...
            try {
                new OdtTableDataExtraction().collectSpecData(args[0]);
            } catch (Exception ex) {
                Logger.getLogger(JarFunctionality.class.getName()).log(Level.SEVERE, null, ex);
            }
        }else {
            for(String path : args){
                try {
                    new OdtTableDataExtraction().collectSpecData(path);
                } catch (Exception ex) {
                    Logger.getLogger(JarFunctionality.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
