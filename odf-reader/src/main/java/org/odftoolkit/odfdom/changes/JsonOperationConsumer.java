...
//                        String type = op.getString(OPK_TYPE);
                    ...
                    String family = StringEscapeUtils.unescapeJson(attrs.optString("family"));
                    ...
                    String type = op.optString(OPK_TYPE);
                    JSONObject attrs = op.optJSONObject(OPK_ATTRS);
                    addField(rootComponent, opsDoc.getDocument().getContentDom(), start, type, representation, attrs);
    ...
    public static void addText(Component rootComponent, JSONArray start, JSONObject attrs, String newText) throws IndexOutOfBoundsException {
        ...
        if (parentComponent != null && start != null) {
            try {
                addText((TextParagraphElementBase) parentComponent.getRootElement(), start.getInt(start.length() - 1), attrs, newText);
            } catch (JSONException ex) {
                Logger.getLogger(JsonOperationConsumer.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
...
//ToDo PERFORMANCE: Sometimes we do have the parentComponentElement, e.g. the table row component earlier, why searching it ALWAYS again?
            ...
            if (parentComponent != null) {
                ...
                LOG.log(Level.SEVERE, "No parent component found to '" + OP_FORMAT + "' from {0} to {1} trying to add {2}", new Object[]{start, end, attrs});
            }
        } else {
            LOG.log(Level.SEVERE, "No parent component found to '" + OP_FORMAT + "' from {0} to {1} trying to add {2}", new Object[]{start, end, attrs});
        }
...
//								}
        ...
        if (newFieldElement != null && currentMap != null && attrs != null && attrs.has("field")) {
            try {
                ...
                if (currentMap.hasId() && fieldAttrs.has(OPK_ID)) {
                    newFieldElement.setAttributeNS(OdfDocumentNamespace.XML.getUri(), "xml:id", fieldAttrs.getString(OPK_ID));
                }
                ...
                if (currentMap.hasHref() && fieldAttrs.has("href")) {
                    newFieldElement.setAttributeNS(OdfDocumentNamespace.XLINK.getUri(), "xlink:href", StringEscapeUtils.unescapeJson(fieldAttrs.getString("href")));
                }
    ...
    public static void addDrawing(Component rootComponent, JSONArray start, JSONObject attrs, String type, CollabTextDocument opsDoc)
        ...
        if (parentComponent == null) {
            ...
            if (attrs != null) {
                ...
                try { //TODO: at the moment only one type of shapes can be inserted - a text frame, later a detection of non-textframes is required here
                    if (/*type.equals("shape") && */!attrs.has(OPK_STYLE_ID)) {
            ...
            if (imageProps != null) {
                ...
                if (imageUrl != null && imageUrl instanceof String) {
                    packagePath = StringEscapeUtils.unescapeJson((String) imageUrl);
                }
            }
    ...
    public static void setColumnsWidth(Component tableComponent, JSONArray start, JSONArray tableGrid, boolean isTextTable) {
        if (tableGrid != null) {
            ...
            if (tableGrid.length() != columnCount) {
                ...
                addColumnAndCellElements(tableComponent, start, tableGrid, Integer.MAX_VALUE, INSERT_BEFORE, -1, isTextTable, existingColumnList, Boolean.TRUE);
            }
        } else {
    ...
    public static void addColumns(Component rootComponent, JSONArray start, JSONArray tableGrid, Integer referenceColumnGridPosition, String insertMode) {
        ...
        if (parentComponent == null) {
            ...
            if (tableGrid != null) {
                ...
                addColumnAndCellElements(parentComponent, start, tableGrid, referenceColumnGridPosition, insertMode, -1, true, existingColumnList, Boolean.FALSE);
                // WORK AROUND for "UNDO COLUMN WIDTH" problem (see JsonOperationConsumer for further changes)
                if (tableElement.isWidthChangeRequired()) {
                    JsonOperationConsumer.setColumnsWidth(tableElement.getComponent(), tableElement.getPosition(), tableElement.popTableGrid(), Boolean.TRUE);
                }
    ...
    // private static void addColumns(Component rootComponent, JSONArray start, JSONObject attrs, int count, int referenceColumn, boolean isTextTable) {	}
    private static void addColumnAndCellElements(Component tableComponent, JSONArray start, JSONArray tableGrid, Integer referenceColumnGridPosition, String insertMode, int newLastColumnNo, boolean isTextTable, List<TableTableColumnElement> existingColumnList, boolean columnCreationOnly) {
        ...
        if (tableGrid != null) {
            ...
            if (columnGridNo == referenceColumnGridPosition) {
                addColumnElement(columnElement, insertMode, columnGridNo, newAbsColumnWidthsGrid, tableGrid);
                columnGridNo++;
            }
        }
        ...
        if (!columnCreationOnly) {
            ...
            if (referenceColumnGridPosition == null) {
                addCellsInRows(tableComponent, start, appendColumnCount, insertMode, appendColumnCount, isTextTable);
            } else if (referenceColumnGridPosition != Integer.MAX_VALUE) { // MAX VALUE used for setAttribute reussage
                addCellsInRows(tableComponent, start, referenceColumnGridPosition, insertMode, appendColumnCount, isTextTable);
            }
        }
    }
    ...
    private static void addCellsInRows(Component tableComponent, JSONArray start, Integer referenceColumnGridPosition, String insertMode, int cellRepetition, boolean isTextTable) {
        ...
        if (referenceColumnGridPosition != Integer.MAX_VALUE) { // MAX VALUE used for setAttribute reussage
            ...
            addNewRowsCells(tableComponent, start, referenceColumnGridPosition, newColumnTargetGridPosition, cellRepetition, isTextTable);
        }
    }
...
//			if ((rowComponent.getChildNode(cellTargetPosition) == null || cellTargetPosition == 1 + cellReferencePosition) && clonedCellElement != null) {
            ...
            addCells(tableComponent.getRootComponent(), destinationCellPosition, null, cellRepetition, rowComponent, clonedCellElement, isTextTable);
//			}
     ...
     * is 1.
     * @param addDefaultCells	(optional) If true, empty cells will be inserted
     * into the new row. The number of inserted cells will be equal to the
     * number of columns in the tableElement, as specified by the tableGrid
     ...
     * Attributes. Note: The attributes addDefaultCells and referenceRow are
     * mutually exclusive.
     */
    public static void addRows(Component rootComponent, JSONArray start, JSONObject attrs, int count, boolean addDefaultCells, int referenceRow, boolean isTextTable) throws IndexOutOfBoundsException, JSONException {
        ...
        if (parentComponent == null) {
            ...
            if (addDefaultCells) {
                if (isTextTable) {
                    addCells(rootComponent, start, CELL_WITH_BORDER_ATTRS, table.getColumnCount(), newRowComponent, null, isTextTable);
                } else {
                    // if there is no reference row, no cells has been cloned from an existing row and basic cells have to be added
                    if (referenceRow <= -1) {
                        addCells(rootComponent, start, null, table.getColumnCount(), newRowComponent, null, isTextTable);
                    }
                }
            }
            if (count > 1) {
...
//		}
            ...
            if (tableElement.isWidthChangeRequired()) {
                ...
                addColumnAndCellElements(tableElement.getComponent(), start, tableElement.popTableGrid(), cellPosition, INSERT_AFTER, -1, true, existingColumnList, Boolean.TRUE);
                tableElement.hasChangedWidth();
            }
...
//						}else{
    ...
    static public void mapProperties(OdfStyleFamily styleFamily, JSONObject attrs, OdfStyleBase style, OdfDocument doc) {
...
//					}
    ...
    static public void mapCharacterProperties(JSONObject attrs, StyleTextPropertiesElement propertiesElement, OdfDocument doc) {
        if (attrs != null) {
            ...
            if (fontName != null && !fontName.isEmpty()) {
                if (!fontNames.contains(fontName)) {
                    ...
                    if (fontName.equals("Andale Mono")) {
                        ...
                        addFontData(doc, "Calibri", null, "Calibri", "swiss", "variable", "2 15 5 2 2 2 4 3 2 4");
                    } else if (fontName.equals("Cambria")) {
                        addFontData(doc, "Cambria", null, "Cambria", "roman", "variable", "2 4 5 3 5 4 6 3 2 4");
                        ...
                        addFontData(doc, "Microsoft YaHei", null, "Microsoft YaHei", "system", "variable", null);
                    } else if (fontName.equals("MS Gothic")) {
                        addFontData(doc, "MS Gothic", null, "MS Gothic", "modern", "fixed", "2 11 6 9 7 2 5 8 2 4");
                    } else if (fontName.equals("MS Mincho")) {
                        addFontData(doc, "MS Mincho", null, "MS Mincho", "modern", "fixed", "2 2 6 9 4 2 5 8 3 4");
                        ...
                        addFontData(doc, "Times New Roman", null, "Times New Roman", "roman", "variable", "2 2 6 3 5 4 5 2 3 4");
            ...
            for (String key : propKeys) {
                ...
                if (key.equals("fill")) {
                    try {
                        ...
                        if (value == null || value.equals(JSONObject.NULL) || (value.has(OPK_TYPE) && (value.isNull(OPK_TYPE) || value.getString(OPK_TYPE).equals("none")))) {
                            if (flyFrame) {
                                propertiesElement.removeAttributeNS(OdfDocumentNamespace.FO.getUri(), "background-color");
                            } else {
                 ...
                 * Mode=through	Side=both   */ /*
                 ...
                 OX API: textWrapSide	Sides where text wraps around the image (only used if textWrapMode is set to 'square', 'tight', or 'through') (1). One of ['both', 'left', 'right', 'largest'	*/ if (key.equals("textWrapMode")) {
                    try {
                        Object value = attrs.get(key);
                        if (value == null || value.equals(JSONObject.NULL)) {
