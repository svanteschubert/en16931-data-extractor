...
//    private final ArrayDeque<ShapeProperties> mShapePropertiesStack;
    ...
    /**
     * Properties for the HEADER_DEFAULT and FOOTER_DEFAULT page area. Defining
     * the page layout
     */
    ...
    private boolean isBlockedSubTree(String uri, String localName, boolean isStart) {
        ...
        if (isStart) {
            ...
            if (/*!mWhitespaceStatusStack.isEmpty() && Component.isTextComponentRoot(uri, localName) || */OdfElement.isIgnoredElement(uri, localName)
                    || ((isMasterPage || Component.isHeaderRoot(uri, localName) || Component.isFooterRoot(uri, localName)) && OdfDocument.OdfMediaType.TEXT.getMediaTypeString() != mMediaType && OdfDocument.OdfMediaType.SPREADSHEET.getMediaTypeString() != mMediaType)) {
                isBlocked = true;
        ...
        if (element instanceof StyleFontFaceElement) {
            ...
            if (fontName != null && !fontName.isEmpty()) {
                ...
                if (!fontNames.contains(fontName)) {
                    mJsonOperationProducer.addFontData(fontName, null, element.getAttributeNS(OdfDocumentNamespace.SVG.getUri(), "font-family"), element.getAttributeNS(OdfDocumentNamespace.STYLE.getUri(), "font-family-generic"), element.getAttributeNS(OdfDocumentNamespace.STYLE.getUri(), "font-pitch"), element.getAttributeNS(OdfDocumentNamespace.SVG.getUri(), "panose-1"));
                    ...
                    if (!isNestedParagraph) {
                        ...
                        if (!mPageBoundObjectsRelocated && !m_cachedPageShapes.isEmpty()) {
                            ...
                            if (isFirstDocumentParagraph && m_cachedPageShapes.size() > 0) {
                                cacheOperation(false, OperationConstants.PARAGRAPH, position, false, hardFormatting, mContextName);
                                ...
                                while (it.hasNext()) {
                                    ...
                                    component.createShapeOperation(this, mComponentStack, component.mDescription,
                                    ...
                                    while (opIter.hasNext()) {
                                        ...
                                        cacheOperation(false, op.mComponentType, start, false, op.mHardFormattingProperties, op.mComponentProperties);
                                    }
                                }
...
//                mCurrentComponent = mCurrentComponent.addChild((TextSectionElement) element);
                    ...
                    // special handling for frames as together with the image child they are a single user component
                    if (element instanceof DrawFrameElement) {
                        ...
                        if (!mComponentStack.isEmpty()) {
                            ...
                            if (comp instanceof ShapeProperties && ((ShapeProperties) comp).getDrawFrameElement() != null) {
...
//                    mShapePropertiesStack.push(shapeProps);
                    ...
                    if (row.hasAttributeNS(OdfDocumentNamespace.TABLE.getUri(), "visibility")) {
                        isVisible = Constants.VISIBLE.equals(row.getAttributeNS(OdfDocumentNamespace.TABLE.getUri(), "visibility"));
                    }
...
//                       Map<String, Object> hardFormatting = mJsonOperationProducer.getHardStyles(telem);
                    ...
                    CommentComponent commentProps = new CommentComponent(mLastComponentPositions, annotationName);
                    ((OdfDocument) mSchemaDoc).addAnnotation(annotationName, ((OfficeAnnotationElement) element));
                    ...
                    cacheOperation(false, OperationConstants.COMMENTRANGE, position, false, null, id, mContextName);
                    mCurrentComponent = mCurrentComponent.createChildComponent(element);
                    element.markAsComponentRoot(true);
                } else {
                    mComponentDepth++;
                    ...
                    try {
                        ...
                        if (autoStyles != null) {
                            ...
                            if (pageLayout != null) {
                                ...
                                headerAttrs = getHeaderFooterAttrs((OdfElement) pageLayout.getChildElement(StyleHeaderStyleElement.ELEMENT_NAME.getUri(), "header-style"));
                                footerAttrs = getHeaderFooterAttrs((OdfElement) pageLayout.getChildElement(StyleFooterStyleElement.ELEMENT_NAME.getUri(), "footer-style"));
                            }
                        }
        ...
        if (qName.equals("office:styles")) {
            if (mFileDom instanceof OdfStylesDom) {
                ...
                if (officeStyles != null) {
                    ...
                    final Iterator<OdfStyle> paragraphStyleIter = officeStyles.getStylesForFamily(OdfStyleFamily.Paragraph).iterator();
                    ...
                    while (paragraphStyleIter.hasNext()) {
                        ...
                        _defaultTabStopWidth = mJsonOperationProducer.triggerStyleHierarchyOps(officeStyles, OdfStyleFamily.Paragraph, paragraphStyleIter.next());
                        if (_defaultTabStopWidth != null) {
                            defaultTabStopWidth = _defaultTabStopWidth;
                        }
                    }
                    final Iterator<OdfStyle> textStyleIter = officeStyles.getStylesForFamily(OdfStyleFamily.Text).iterator();
                    while (textStyleIter.hasNext()) {
                        mJsonOperationProducer.triggerStyleHierarchyOps(officeStyles, OdfStyleFamily.Text, textStyleIter.next());
                    }
                    final Iterator<OdfStyle> graphicStyleIter = officeStyles.getStylesForFamily(OdfStyleFamily.Graphic).iterator();
                    while (graphicStyleIter.hasNext()) {
                        mJsonOperationProducer.triggerStyleHierarchyOps(officeStyles, OdfStyleFamily.Graphic, graphicStyleIter.next());
                    }
...
//    			}
                    final Iterator<OdfTextListStyle> textListStyleIter = officeStyles.getListStyles().iterator();
                    ...
                    mJsonOperationProducer.addDocumentProperties(stylesDom, defaultTabStopWidth, defaultPageStyles);
                } else {
        ...
        if (!checkEndOfBlockedSubTree(uri, localName) /*&& !(mContextName != null && localName.equals("annotation-end"))*/) {
            ...
            if ((uri != null && uri.equals(DrawFrameElement.ELEMENT_NAME.getUri()) && localName.equals(DrawFrameElement.ELEMENT_NAME.getLocalName()) || Component.isShapeElement(uri, localName))) {
                if (!mComponentStack.empty()) {
                    ...
                    shapeProps.createShapeOperation(this, mComponentStack, description,
                    ...
                    if (shapeProps.isGroupShape()) {
                        mCurrentNode.setUserData("groupWidth", new Integer(shapeProps.mHoriOffsetMax - (shapeProps.mHoriOffsetMin == null ? 0 : shapeProps.mHoriOffsetMin)), null);
                        mCurrentNode.setUserData("groupHeight", new Integer(shapeProps.mVertOffsetMax - (shapeProps.mVertOffsetMin == null ? 0 : shapeProps.mVertOffsetMin)), null);
                    }
                                ...
                                if (s.hasUrl() || styleId != null) {
                                    try {
                                        ...
                                        if (s.hasUrl()) {
                                            ...
                                            charProps.put("url", StringEscapeUtils.escapeJson(s.getURL()));
                                        ...
                                        Logger.getLogger(ChangesFileSaxHandler.class
                                                .getName()).log(Level.SEVERE, null, ex);
                                    }
                                }
                                if (hardFormatting != null) {
//                                    if (mWithinTable) {
                                    cacheOperation(false, OperationConstants.ATTRIBUTES, s.getStartPosition(), false, hardFormatting, s.getEndPosition(), mContextName);
                                }
                            }
                        }
...
//						mWhitespaceStatusStack.pop();
                    ...
                    if (localName.equals("annotation")) {
                        ...
                        if (!commProps.isInHeaderFooter()) {
                            ...
                            cacheOperation(false, OperationConstants.COMMENT, commProps.getComponentPosition(), false, null, id, commProps.getAuthor(), commProps.getDate(), mContextName);
                            ...
                            for (CachedOperation op : commProps) {
                                ...
                                int propIndex = 0;
                                while (newOp.mComponentProperties.length > propIndex && newOp.mComponentProperties[propIndex] != null) {
                                    if (propIndex == 0 && newOp.mComponentType.equals(OperationConstants.ATTRIBUTES)) {
                                        @SuppressWarnings("unchecked")
                                ...
                                cacheOperation(false, newOp.mComponentType, newOp.mStart, true, newOp.mHardFormattingProperties, componentProperties.toArray());
                            }
                        }
//                    } else if (localName.equals("annotation-end")){ //no action required
                    }
                            ...
                            if (pos > 0) {
                                ...
                                for (int rep = 0; rep < repetition - 1; ++rep) {
                                    for (int opPos = pos; opPos < opSize; ++opPos) {
                                        ...
                                        cacheOperation(false, newOp.mComponentType, newOp.mStart, false, newOp.mHardFormattingProperties, newOp.mComponentProperties);
                                    }
            ...
            if (attr != null) {
                ...
                try {
                    ...
                    if (errorHandler != null) {
                        try {
                            errorHandler.error(new OdfValidationException(OdfSchemaConstraint.DOCUMENT_XML_INVALID_ATTRIBUTE_VALUE, attr.getValue(), attr.getPrefix() + ":" + attr.getLocalName()));
        ...
        if (mCharsForElement.length() > 0) {
            ...
            if (isEndOfElement && Component.isField(uri, localName)) {
                ...
                if (!isBlockedSubTree()) {
                    ...
                    cacheOperation(false, OperationConstants.FIELD, textSelection.getStartPosition(), false, null, localName, replacementText != null ? replacementText : newString, attrMap, mContextName);
                }
...
//                    if ((Component.isTextComponentRoot(mCurrentNode) || Component.isTextComponentRoot(((OdfElement) mCurrentNode).getComponentRoot())) && !isBlockedSubTree() && mWhitespaceStatusStack.size() > 0 && !mWhitespaceStatusStack.getLast().isParagraphIgnored() && !(mCurrentNode instanceof TextNoteCitationElement)) {
            ...
            if (isEndOfElement && Component.isField(uri, localName)) {
                ...
                if (!isBlockedSubTree()) {
                    ...
                    cacheOperation(false, OperationConstants.FIELD, textSelection.getStartPosition(), false, null, localName, replacementText, attrMap, mContextName);
                }
            }
        }
    ...
    private void addText(CharSequence newText) {
        cacheOperation(false, OperationConstants.TEXT, mCharsStartPosition, false, null, newText.toString(), mContextName);
    ...
    public void cacheOperation(boolean fillCacheOnly, String componentType, List<Integer> start, boolean absolutePosition, Map<String, Object> hardFormattingProperties, Object... componentProperties) {
        if (mComponentStack.empty()) {
            ...
            if (componentType.equals(OperationConstants.TEXT)) {
                ...
                mJsonOperationProducer.addTable(start, hardFormattingProperties, tableGrid, tableName, context);
            ...
            } else if (componentType.equals(OperationConstants.SHAPE) || componentType.equals(OperationConstants.SHAPE_GROUP)) {
                String context = (String) componentProperties[0];
                mJsonOperationProducer.addShape(start, hardFormattingProperties, context, componentType.equals(OperationConstants.SHAPE_GROUP));
                ...
                mJsonOperationProducer.add(componentType, start, hardFormattingProperties, target != null ? target : mContextName);
            } else {
                String target = (String) componentProperties[0];
                mJsonOperationProducer.add(componentType, start, hardFormattingProperties, target != null ? target : mContextName);
            }
    ...
    private void /*CachedTable */ cacheTableOperation(/*CachedTable currentTable,*/String componentType, List<Integer> start, Map<String, Object> hardFormattingProperties, Object... componentProperties) {
        ...
        if (componentType.equals(OperationConstants.CELLS)) { // does not effect a spreadsheet
            ...
            if (newCachedTable.mTableGrid != null) {
                if (mMaxAllowedColumnCount != 0 && newCachedTable.mTableGrid.size() > mMaxAllowedColumnCount) {
...
//                ++currentTable.mNumberOfNestedTables;
        ...
        if (!currentTable.mIsTooLarge && !componentType.equals(OperationConstants.TABLE) && !componentType.equals(OperationConstants.COLUMNS)) {
            currentTable.add(new CachedInnerTableOperation(componentType, position, false, hardFormattingProperties, componentProperties));
        }
    }
            ...
            if (operation instanceof CachedInnerTableOperation && operation.mComponentType.equals(OperationConstants.TABLE)) {
                if (isStartOfTable) {
                    ...
                    if (currentTable.mIsTooLarge) {
                        // replacement table
                        cacheOperation(false, OperationConstants.EXCEEDEDTABLE, operation.mStart, false, null, ((List) operation.mComponentProperties[0]).size(), currentTable.mRowCount, operation.mComponentProperties[0], mContextName);
                        break;
                    } else {
                        if ((mMaxAllowedRowCount != 0 && currentTable.mRowCount > mMaxAllowedRowCount)
                                || (mMaxAllowedColumnCount != 0 && currentTable.mColumnCount > mMaxAllowedColumnCount)
                            ...
                            cacheOperation(false, OperationConstants.TABLE, operation.mStart, false, operation.mHardFormattingProperties, operation.mComponentProperties[0], operation.mComponentProperties[1], mContextName);
                        }
                    }
                } else {
                ...
                cacheOperation(false, operation.mComponentType, operation.mStart, false, null, operation.mComponentProperties[0], context);
            } else if (operation.mComponentType.equals(OperationConstants.ATTRIBUTES)) {
                String context = mContextName;
                if (operation.mComponentProperties.length > 1 && operation.mComponentProperties[1] != null) {
                    context = (String) operation.mComponentProperties[1];
                }
                cacheOperation(false, OperationConstants.ATTRIBUTES, operation.mStart, false, operation.mHardFormattingProperties, operation.mComponentProperties[0], context);
            } else if (operation.mComponentType.equals(OperationConstants.SHAPE) || operation.mComponentType.equals(OperationConstants.IMAGE) || operation.mComponentType.equals(OperationConstants.SHAPE_GROUP)) {
                cacheOperation(false, operation.mComponentType, operation.mStart, false, operation.mHardFormattingProperties, mContextName);
                ...
                cacheOperation(false, operation.mComponentType, operation.mStart, false, null, operation.mComponentProperties[0], operation.mComponentProperties[1], attrMap, mContextName);
                ...
                cacheOperation(false, operation.mComponentType, operation.mStart, false, operation.mHardFormattingProperties, operation.mComponentProperties);
            } else if (operation.mComponentType.equals(OperationConstants.COMMENT) || operation.mComponentType.equals(OperationConstants.COMMENTRANGE)) {
                cacheOperation(false, operation.mComponentType, operation.mStart, false, operation.mHardFormattingProperties, operation.mComponentProperties);
            } else {
                boolean isParagraphOperation = operation.mComponentType.equals(OperationConstants.PARAGRAPH);
                ...
                cacheOperation(false, operation.mComponentType, operation.mStart, false, operation.mHardFormattingProperties, context);
            }
        }
    }
    ...
    private JSONObject getHeaderFooterAttrs(OdfElement e) {
        ...
        if (e != null) {
            final Element p = e.getChildElement(StyleHeaderFooterPropertiesElement.ELEMENT_NAME.getUri(), "header-footer-properties");
            if (p != null) {
     ...
     *
     * private CachedTable evaluateSimilarCells(CachedTable tableOps,
     * CachedInnerTableOperation cellOperation, JSONObject currentCell, boolean
     * isRow) { // An Operation will always be triggered in the end of the
     * function boolean triggerOperation = false;
     *
     * // every repeatedColumns row will result into a fillRange operation int
     * previousContentRepetition = 1;
     *
     * // if the previous cells are equal if (tableOps.mFirstEqualCellNo > -1)
     * { previousContentRepetition = tableOps.mCurrentColumnNo -
     * tableOps.mFirstEqualCellNo; }
     *
     * boolean isRepeatedRow = tableOps.mLastRow != null &&
     * !tableOps.mFirstRow.equals(tableOps.mLastRow);
     *
     * // do not trigger the operation if the spreadsheetRow is null and its
     * only member is null if (tableOps.mSheetNo == null && cellOperation !=
     * null && cellOperation.mStart != null) { // we have a cell position and
     * require the two above (first parent row, afterwards cell) => already -2
     * // and an additional - 1 as size of 1 would result in zero position ==>
     * finally -3 tableOps.mSheetNo =
     * cellOperation.mStart.get(cellOperation.mStart.size() - 3); } // ** There
     * are four variations for previous/current cell we have to check: // 1)
     * Current Content Cell, Previous Content empty if (currentCell != null &&
     * tableOps.mPreviousCell != null) { // if the two cells are NOT the same if
     * (!currentCell.equals(tableOps.mPreviousCell)) { if
     * (previousContentRepetition > MIN_REPEATING_CONTENT_CELLS) {
     * triggerOperation = true; } else { if (tableOps.mCurrentRange == null) {
     * tableOps.mCurrentRange = new JSONArray(); } // Resolving
     ...
     * tableOps.mCurrentRange.put(tableOps.mPreviousCell); } // if the row is
     * being repeated, there are always vertical spans (fill same content
     * multiple times if (tableOps.mLastRow - tableOps.mFirstRow > 0) {
     * triggerOperation = true; } // there is an upcoming fill operation, the
     * previous content has to be flushed if (tableOps.getCellRepetition() >
     * MIN_REPEATING_CONTENT_CELLS) { triggerOperation = true; }
     * tableOps.mFirstEqualCellNo = tableOps.mCurrentColumnNo; } } // 2) Current
     * Content Cell, Previous Empty Cell (never have saved anything) } else if
     * (currentCell != null && tableOps.mPreviousCell == null) { // &&
     * tableOps.mCurrentRange == null tableOps.mFirstEqualCellNo =
     * tableOps.mCurrentColumnNo; if (tableOps.mFirstContentCellNo == -1) { //
     * reset the empty cell counter - if previous was empty
     * tableOps.mFirstContentCellNo = tableOps.mCurrentColumnNo;
     * tableOps.mEmptyCellCount = 0; } else { if (tableOps.getCellRepetition() >
     * MIN_REPEATING_CONTENT_CELLS) { triggerOperation = true; } else { if
     * (tableOps.mCurrentRange == null) { tableOps.mCurrentRange = new
     * JSONArray(); } for (int i = 0; tableOps.mEmptyCellCount > i; i++) {
     * tableOps.mCurrentRange.put(JSONObject.NULL); } tableOps.mEmptyCellCount =
     * 0; } } // 3) Content Cell empty, Previo Cell full } else if (currentCell
     * == null && tableOps.mPreviousCell != null) { tableOps.mEmptyCellCount +=
     * tableOps.getCellRepetition(); // as there had been previously content //
     * check if it was repeating content if (previousContentRepetition >
     * MIN_REPEATING_CONTENT_CELLS) { triggerOperation = true; } else { if
     * (tableOps.mCurrentRange == null) { tableOps.mCurrentRange = new
     * JSONArray(); } // save the previous cell for later compressed output for
     * (int i = 0; tableOps.mCurrentColumnNo - tableOps.mFirstEqualCellNo > i;
     * i++) { tableOps.mCurrentRange.put(tableOps.mPreviousCell); } // if the
     * row is being repeated, there are always vertical spans (fill same content
     * multiple times if (tableOps.mLastRow - tableOps.mFirstRow > 0) {
     * triggerOperation = true; } } tableOps.mFirstEqualCellNo = -1; // if there
     * was previously repeating content cells // 4) Both are null } else if
     * (currentCell == null && tableOps.mPreviousCell == null & !isRow) { //
     * note that an empty cell was passed tableOps.mEmptyCellCount +=
     * tableOps.getCellRepetition(); // if this is the first empty cell if
     * (tableOps.mFirstEmptyCell == -1) { // remember when it started
     * tableOps.mFirstEmptyCell = tableOps.mCurrentColumnNo;
     *
     * // else check if the maximum repeated empty cells was reached and
     * existing content has to be dispatched as an operation } else if
     * (tableOps.mFirstContentCellNo != -1 && MAX_REPEATING_EMPTY_CELLS >
     * tableOps.mEmptyCellCount) { triggerOperation = true; } }
     *
     * // RANGE CREATION: for every row we flush previous content OR if we want
     * to flush for other reasons if (isRow && tableOps.mFirstContentCellNo > -1
     * || triggerOperation) { // WRITING WHITESPACE TO ROW // if the last cell
     * used content, but there was previous whitespace, the whitespace has to be
     * explicitly set if (tableOps.mEmptyCellCount > 0 && currentCell != null) {
     * for (int i = 0; tableOps.mEmptyCellCount > i; i++) { if
     * (tableOps.mCurrentRange == null) { tableOps.mCurrentRange = new
     * JSONArray(); } tableOps.mCurrentRange.put(JSONObject.NULL); }
     * tableOps.mEmptyCellCount = 0; } // WRITING CELL TO ROW // if content to
     * flush exist and the operation was triggered // OR there is horizontal
     * repeated content // OR there is vertical repeated content if
     * (tableOps.mCurrentRange != null || previousContentRepetition >
     * MIN_REPEATING_CONTENT_CELLS || isRepeatedRow) { if(tableOps.mCurrentRange
     * != null && !tableOps.mCurrentRange.isEmpty()) { Component rootComponent =
     * mSchemaDoc.getRootComponent(); TableTableElement sheet =
     * (TableTableElement)rootComponent.getChildNode(tableOps.mSheetNo); }
     *
     * mJsonOperationProducer.addRange(tableOps.mSheetNo, tableOps.mFirstRow,
     * tableOps.mLastRow, tableOps.mPreviousRepeatedRows,
     * tableOps.mFirstContentCellNo, previousContentRepetition,
     * tableOps.mPreviousCell, tableOps.mCurrentRange, previousContentRepetition
     * > MIN_REPEATING_CONTENT_CELLS); } // if a fill sufficent repeating is now
     * after a content, the previous content was flushed if
     * (tableOps.mFirstEqualCellNo == tableOps.mCurrentColumnNo) { // but still
     * a content and repeating content exits tableOps.mFirstContentCellNo =
     * tableOps.mFirstEqualCellNo; } else { if (currentCell != null) {
     * tableOps.mFirstContentCellNo = tableOps.mCurrentColumnNo;
     * tableOps.mFirstEqualCellNo = tableOps.mCurrentColumnNo; } else {
     * tableOps.mFirstContentCellNo = -1; tableOps.mFirstEqualCellNo = -1; } }
     * tableOps.mCurrentRange = null; } if (!isRow) { // Making the current cell
     * the previous for next round tableOps.mPreviousCell = currentCell;
     * tableOps.mCurrentColumnNo += tableOps.getCellRepetition();
     * tableOps.setCellRepetition(1); } else { // after the end of a row reset
     * all values tableOps.mSheetNo = null; tableOps.mCurrentRange = null;
     * tableOps.mPreviousCell = null; tableOps.mFirstContentCellNo = -1;
     * tableOps.mFirstEqualCellNo = -1; tableOps.mCurrentColumnNo = 0;
     * tableOps.mEmptyCellCount = 0; tableOps.mFirstEmptyCell = -1;
     ...
     * getTableColumnElements(tableElement, new
     ...
     *
     * static List<Integer> collectColumnWidths(TableTableElement tableElement,
     * List<TableTableColumnElement> columns) { boolean hasRelColumnWidth =
     * false; boolean hasAbsColumnWidth = false; boolean hasColumnWithoutWidth =
     * false; List<Integer> columnRelWidths = new ArrayList(); for
     * (TableTableColumnElement column : columns) { if
     * (column.hasAttributeNS(OdfDocumentNamespace.TABLE.getUri(),
     * "style-name")) { Length tableWidth =
     * getPropertyLength(StyleTablePropertiesElement.Width, tableElement);
     *
     * int repeatedColumns = 1; if
     * (column.hasAttributeNS(OdfDocumentNamespace.TABLE.getUri(),
     ...
     *
     * // it is being assumed, when the columnRelWidth is once set, it is
     * always set if (columnRelWidth != null && !columnRelWidth.isEmpty()) {
     * hasRelColumnWidth = true; if (hasAbsColumnWidth) { LOG.warning("*******
     * BEWARE: Absolute and relative width are not supposed to be mixed!!
     * ***********"); } columnRelWidth = columnRelWidth.substring(0,
     * columnRelWidth.indexOf('*')); Integer relWidth =
     * Integer.parseInt(columnRelWidth); for (int i = 0; i < repeatedColumns;
     * i++) { columnRelWidths.add(relWidth); } } else { // if there is no
     * relative column width if (hasRelColumnWidth) { LOG.warning("*******
     * BEWARE: Absolute and relative width are not supposed to be mixed!!
     * ***********"); }
     *
     * Length columnWidth =
     * getPropertyLength(StyleTableColumnPropertiesElement.ColumnWidth, column);
     * // there can be only table width and .. if (tableWidth != null) { //
     * columnwidth, with a single one missing if (columnWidth != null) {
     * hasAbsColumnWidth = true; int widthFactor = (int)
     * Math.round((columnWidth.getMillimeters() * 100) /
     * tableWidth.getMillimeters()); for (int i = 0; i < repeatedColumns; i++) {
     * columnRelWidths.add(widthFactor); } } else { if (hasColumnWithoutWidth) {
     * LOG.warning("******* BEWARE: Two columns without width and no column
     * width are not expected!! ***********"); } hasColumnWithoutWidth = true; }
     * // if the table is not set, it will always be unset.. } else { if
     * (columnWidth != null) { hasAbsColumnWidth = true; int widthFactor = (int)
     * Math.round((columnWidth.getMicrometer() * 10)); for (int i = 0; i <
     * repeatedColumns; i++) { columnRelWidths.add(widthFactor); } } else {
     * LOG.warning("******* BEWARE: Two columns without width and no column
     * width are not expected!! ***********"); } } } } } return columnRelWidths;
     ...
     *
     * static List<TableTableColumnElement> getTableColumnElements(Element
     * parent, List columns) { NodeList children = parent.getChildNodes(); for
     * (int i = 0; i < children.getLength(); i++) { Node child =
     * children.item(i); if (child instanceof Element) { if (child instanceof
     * TableTableColumnElement) { columns.add(child); } else if (child
     * instanceof TableTableColumnGroupElement || child instanceof
     * TableTableHeaderColumnsElement || child instanceof
     * TableTableColumnsElement) { columns = getTableColumnElements((Element)
     * child, columns); } else if (child instanceof TableTableRowGroupElement ||
     * child instanceof TableTableHeaderRowsElement || child instanceof
     * TableTableRowElement || child instanceof TableTableRowsElement) { break;
     * } } } return columns; }
     *
     */
}
