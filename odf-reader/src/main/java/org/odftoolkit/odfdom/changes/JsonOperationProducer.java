...
/**
 * The names of operations / changes
 */
...
//			}
    ...
    public void addAnnotation(final List<Integer> start, String id, String author, String date, String context) {
    ...
    /**
     * Maps the ODF office:value-type attribute to the Changes API Text
...
//			} else if (valueType.equals("A")) {
    ...
    public void addText(final List<Integer> start, final String text, final String context) {
        ...
        try {
            newOperation.put("text", StringEscapeUtils.escapeJson(text));
     ...
     * @return
     */
    public void format(final List<Integer> start, final List<Integer> end, Map<String, Object> attrs, String context) {
...
//			}
    ...
    public void addField(final List<Integer> start, String fieldType, String fieldContent, final Map<String, Object> fieldAttributes, String context) {
        final JSONObject newOperation = new JSONObject();
    ...
    /**
     * *
     * addStylesheet name: 'addStylesheet' type: "table" styleId: The identifier
     * of this stylesheet (unique for the corresponding type). stylename: The
     * readable name of this style sheet. attrs: JSONObject, contains formatting
     * attributes as nested JSON objects, keyed by attribute family, as well as
     * other properties specific to a family. Must support the attributes of the
     * main attribute family, may support attributes of other families. See
     * chapter "Style Sheet Properties" below for a list of supported values.
     * parent: (string, optional) The identifier of the parent style sheet that
     * derives formatting attributes to this style sheet (default: no parent).
     * hidden: (boolean, optional) Whether the style sheet is hidden in the user
     * interface (default: false). uipriority: (integer, optional) The priority
     * of the style sheet used to order style sheet in the user interface. The
     * lower the value the higher the priority (default: 0). default: (boolean,
     * optional) Whether this style sheet is the default style sheet of the
     * family. Only one style sheet per family can be the default style sheet
     * (default: false). pooldefault: (boolean, optional) Whether this style
     * sheet is the pool default style sheet. (default: false). OOXML may have
     * on the table default style properties for table Object { type: table
     * properties }	'' row	Object { type: row properties } '' cell	Object {
     * type: cell properties }	'' paragraph	Object { type: paragraph properties
     * }	'' character	Object { type: character properties } We need to split
     * this table style into an additional row and cell style and addChild it as
     * parent for those.
     */
    //
     ...
     * font substitution algorithms. name	String	'addFontData'
     *
    ...
    public void addListStyle(TextListStyleElement listStyle) {
        if (listStyle != null) {
            ...
            if (!knownListStyles.containsKey(styleId)) {
                ...
                addListStyle(listStyle.getStyleNameAttribute(), listStyle.getStyleDisplayNameAttribute(), listStyle.getTextConsecutiveNumberingAttribute(), getListLevelDefinitions(listStyle));
            }
        }
    }
    ...
    private JSONObject createListLevelDefinition(TextListLevelStyleElementBase listLevelStyle, int listLevel) throws JSONException {
        ...
        if (listLevelStyle.hasAttributeNS(OdfDocumentNamespace.TEXT.getUri(), "style-name")) {
            listLevelDefinition.put(OPK_STYLE_ID, listLevelStyle.getAttributeNS(OdfDocumentNamespace.TEXT.getUri(), "style-name"));
        }
...
//									}
    ...
    // addListStyle(knownListStyles.size(), listStyle.getStyleNameAttribute(), listStyle.getStyleDisplayNameAttribute(), listStyle.getTextConsecutiveNumberingAttribute(), getListLevelDefinitions(listStyle));
...
//			newOperation.put("displayName", displayName);
                ...
                if (!knownStyles.containsKey(((OdfStyle) style).getStyleNameAttribute())) {
                    ...
                    for (int i = parents.size() - 1; i >= 0; i--) {
                        ...
                        if (parentStyleName != null && !parentStyleName.isEmpty()) {
                            addStyleSheet(styleName, familyId, ((OdfStyle) style).getStyleDisplayNameAttribute(), mappedFormatting, parentStyleName, nextStyle, outlineLevel, false, false, custom);
                        } else {
                            // the template style without parent is a root style and being used as a default style in the editor
                            //addStyleSheet(styleName, familyId, ((OdfStyle) style).getStyleDisplayNameAttribute(), mappedFormatting, ODFTK_DEFAULT_STYLE_PREFIX + familyId + ODFTK_DEFAULT_STYLE_SUFFIX, nextStyle, outlineLevel, true, false);
                            addStyleSheet(styleName, familyId, ((OdfStyle) style).getStyleDisplayNameAttribute(), mappedFormatting, Constants.ODFTK_DEFAULT_STYLE_PREFIX + familyId + Constants.ODFTK_DEFAULT_STYLE_SUFFIX, nextStyle, outlineLevel, false, false, custom);
                        }
     ...
     * triggers a addStylesheet operation
     */
    public Integer triggerDefaultStyleOp(OdfStyleFamily styleFamily, OdfStyleBase style) {
        ...
        if (style instanceof OdfDefaultStyle && !knownStyles.containsKey(Constants.ODFTK_DEFAULT_STYLE_PREFIX + Component.getFamilyID(styleFamily) + Constants.ODFTK_DEFAULT_STYLE_SUFFIX)) {
            ...
            if (!mappedFormatting.isEmpty()) {
                ...
                addStyleSheet(Constants.ODFTK_DEFAULT_STYLE_PREFIX + familyId + Constants.ODFTK_DEFAULT_STYLE_SUFFIX, familyId, displayName, mappedFormatting, null, null, null, true, true, null);
            }
            // addChild named style to known styles, so it will be only executed once
                ...
                if (styleName != null && !styleName.isEmpty()) {
                    ...
                    if (pageLayoutName != null && !pageLayoutName.isEmpty()) {
                        ...
                        if (autoStyles != null) {
                            ...
                            /**
                             * SVANTEWHY NO LONGER REQUIRED NodeList
                             * pageLayoutChildren = pageLayout.getChildNodes();
                             * if (pageLayoutChildren.getLength() > 1) { for
                             * (int i = 0; i < pageLayoutChildren.getLength();
                             * i++) { Node child = pageLayoutChildren.item(i);
                             * boolean isHeaderAttribute = child instanceof
                             * StyleHeaderStyleElement; if (isHeaderAttribute ||
                             * child instanceof StyleFooterStyleElement) {
                             * StyleHeaderFooterPropertiesElement props =
                             * (StyleHeaderFooterPropertiesElement)
                             * ((OdfElement)
                             * child).getChildElement(StyleHeaderFooterPropertiesElement.ELEMENT_NAME.getUri(),
                             * StyleHeaderFooterPropertiesElement.ELEMENT_NAME.getLocalName());
                             * if (props != null) { if(isHeaderAttribute) {
                             * String marginHeader =
                             * props.getFoMarginBottomAttribute(); String
                             * minHeightHeader =
                             * props.getFoMinHeightAttribute(); String
                             * fixedHeightHeader =
                             * props.getSvgHeightAttribute();
                             * if(fixedHeightHeader != null) minHeightHeader =
                             * fixedHeightHeader; if (marginHeader != null &&
                             * !marginHeader.isEmpty()) { if (pagePropsJson ==
                             * null) { pagePropsJson = new JSONObject(); } try {
                             * int minHeightNormalized = 0; if (minHeightHeader
                             * != null && !minHeightHeader.isEmpty()) {
                             * minHeightNormalized =
                             * MapHelper.normalizeLength(minHeightHeader); }
                             * pagePropsJson.put("marginHeader",
                             * MapHelper.normalizeLength(marginHeader) +
                             * minHeightNormalized); } catch (JSONException ex)
                             * {
                             * Logger.getLogger(JsonOperationProducer.class.getName()).log(Level.SEVERE,
                             * null, ex); } } } else { String marginFooter =
                             * props.getFoMarginTopAttribute(); String
                             * fixedHeightFooter =
                             * props.getSvgHeightAttribute(); String
                             * minHeightFooter =
                             * props.getFoMinHeightAttribute(); if(
                             * fixedHeightFooter != null ) { minHeightFooter =
                             * fixedHeightFooter; } if (marginFooter != null &&
                             * !marginFooter.isEmpty()) { if (pagePropsJson ==
                             * null) { pagePropsJson = new JSONObject(); } try {
                             * int minHeightNormalized = 0; if (minHeightFooter
                             * != null && !minHeightFooter.isEmpty()) {
                             * minHeightNormalized =
                             * MapHelper.normalizeLength(minHeightFooter); }
                             * pagePropsJson.put("marginFooter",
                             * MapHelper.normalizeLength(marginFooter) +
                             * minHeightNormalized); } catch (JSONException ex)
                             * {
                             * Logger.getLogger(JsonOperationProducer.class.getName()).log(Level.SEVERE,
                             * null, ex); } } } } } } } SVANTE REQUIRED
                             */
                            if (pagePropsJson != null && pagePropsJson.length() != 0) {
                                if (styleName.equals("Standard") || styleName.equals("MP0")) {
                                    ...
                                    /**
                                     * SVANTE NO LONGER REQUIRED try { //make
                                     * values fit for the frontend
                                     * if(pagePropsJson.has("marginHeader")) {
                                     * int headerMargin =
                                     * pagePropsJson.getInt("marginHeader"); int
                                     * topMargin =
                                     * pagePropsJson.getInt("marginTop");
                                     * pagePropsJson.put("marginTop",
                                     * headerMargin + topMargin);
                                     * pagePropsJson.put("marginHeader",
                                     * topMargin); }
                                     * if(pagePropsJson.has("marginFooter")) {
                                     * int footerMargin =
                                     * pagePropsJson.getInt("marginFooter"); int
                                     * bottomMargin =
                                     * pagePropsJson.getInt("marginBottom");
                                     * pagePropsJson.put("marginBottom",
                                     * footerMargin + bottomMargin);
                                     * pagePropsJson.put("marginFooter",
                                     * bottomMargin); } } catch (JSONException
                                     * e) { } break; } else { //
                                     * documentPropsObject.put("page",
                                     * pagePropsJson); SVANTE REQUIRED
                                     */
                                }
                            }
                        }
