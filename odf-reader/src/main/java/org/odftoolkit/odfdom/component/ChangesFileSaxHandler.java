...
//            mIsParagraphIgnored = isParagraphIgnored;
    ...
    public ChangesFileSaxHandler(Node rootNode) throws SAXException {
       ...
       // Make the root of component tree (to be created) accessible via the ODF schema document
        ...
        mMaxAllowedColumnCount = OperationConstants.MAX_SUPPORTED_COLUMNS;
        mMaxAllowedRowCount = OperationConstants.MAX_SUPPORTED_ROWS;
        mMaxAllowedCellCount = OperationConstants.MAX_SUPPORTED_CELLS;
...
//                mCurrentComponent = mCurrentComponent.addChild((TextSectionElement) element);
                    ...
                    if (shape instanceof DrawShapeElementBase) {
                        ...
                        if (null != zIndex) { anchorLayerOrder = zIndex; }
                    }
...
//                    mShapePropertiesStack.push(shapeProps);
                    ...
                    cacheOperation(false, OperationConstants.HARD_BREAK, position, false, null, null, null);
                    element.markAsComponentRoot(true);
...
//                       Map<String, Object> hardFormatting = mJsonOperationProducer.getHardStyles(telem);
                ...
                mJsonOperationProducer.insertHeaderFooter(mContextName, pageArea, headerAttrs);
                ...
                mContextName = pageArea.getPageAreaName() + CONTEXT_DELIMITER + mMasterPageStyleName;
                mJsonOperationProducer.insertHeaderFooter(mContextName, pageArea, footerAttrs);
...
//    			}
    ...
    //                ShapeProperties shapeProps = mShapePropertiesStack.removeLast();
                    ...
                    boolean isTopLevelAndPageBound = (mCurrentNode.getParentNode().getNamespaceURI().equals(OfficeTextElement.ELEMENT_NAME.getUri()) && mCurrentNode.getParentNode().getLocalName().equals(OfficeTextElement.ELEMENT_NAME.getLocalName()));
	...
	public void cacheOperation(boolean fillCacheOnly, String componentType, List<Integer> start, boolean absolutePosition, Map<String, Object> hardFormattingProperties, Object... componentProperties) {
    	if(mComponentStack.empty()) {
    		...
    		if( componentType.equals(OperationConstants.TEXT)) {
                ...
                mJsonOperationProducer.insertComment(start, id, author, date, target);
            } else if( componentType.equals(OperationConstants.COMMENTRANGE)){
                String id = (String)componentProperties[0];
                String target = (String)componentProperties[1];
                mJsonOperationProducer.insertCommentRange(start, id, target);
    ...
    private CachedTable evaluateSimilarCells(CachedTable tableOps, CachedInnerTableOperation cellOperation, JSONObject currentCell, boolean isRow) {
        ...
        if (currentCell != null && tableOps.mPreviousCell != null) {
            ...
            if (!currentCell.equals(tableOps.mPreviousCell)) {
                if (previousContentRepetition > MIN_REPEATING_CONTENT_CELLS) {
                    ...
                    if (tableOps.getCellRepetition() > MIN_REPEATING_CONTENT_CELLS) {
                        triggerOperation = true;
                    }
            ...
            tableOps.mEmptyCellCount += tableOps.getCellRepetition();
        ...
        if (isRow && tableOps.mFirstContentCellNo > -1 || triggerOperation) {
            ...
            if (tableOps.mCurrentRange != null || previousContentRepetition > MIN_REPEATING_CONTENT_CELLS || isRepeatedRow) {
                if(tableOps.mCurrentRange != null && !tableOps.mCurrentRange.isEmpty()) {
                    Component rootComponent = mSchemaDoc.getRootComponent();
